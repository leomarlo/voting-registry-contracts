...
async function deploy(what:string, verbosity: number, gasPrice: BigNumber){
    if (what=='registry'){
        await deployRegistry(verbosity, gasPrice)
    } else if (what=='registrarAndResolver'){
      await deployRegistrarAndResolver(verbosity, gasPrice)
    } else if (what=='nftWeightedVotingContract') {
