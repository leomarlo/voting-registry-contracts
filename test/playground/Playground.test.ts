...
describe("Playground", function(){
    ...
    describe("Write Operations That require Token Weighted Votes", function(){
        ...
        beforeEach(async function(){
            ...
            await contracts.playground.connect(Alice).start(votingParamsTemp, callbackGuru)
            await contracts.playground.connect(Alice).start(votingParamsTemp, callbackPriest)
            await contracts.playground.connect(Alice).vote(0, APPROVE)
            await contracts.playground.connect(Alice).vote(1, APPROVE)
            await contracts.playground.connect(Bob).vote(0, APPROVE)
            await contracts.playground.connect(Bob).vote(1, APPROVE)
            let timestampTemp = (await ethers.provider.getBlock(await ethers.provider.getBlockNumber())).timestamp
            await ethers.provider.send('evm_setNextBlockTimestamp', [timestampTemp + durationtemp + 3]); 
            ...
            expect(await contracts.playground.getIncumbentFromOffice(newOffice)).to.equal(Alice.address)
            expect(await contracts.playground.getIncumbentFromOffice(anotheroffice)).to.equal(Bob.address)
        })
        it("Should deploy a new Contract", async function(){
            let duration: number = types.deployNewContract.duration as number
            let votingParams = abi.encode(
                ["address", "uint256", "bool"],
                [contracts.playground.address, minQuorum, expectReturnValue])
            callback = playgroundInterface.encodeFunctionData("deployNewContract", 
                [abi.encode(["uint256"],[1001920]), ERC20Factory.bytecode])
            ...
            console.log(await contracts.playground.votingMetaParams(callback.slice(0,10)))
            console.log(`Quorum is ${minQuorum}. Duration is ${duration}.`)
            let identifier = await contracts.majorityWithoutToken.getCurrentIndex()
            let sigHash = playgroundInterface.getSighash("start")
            let tokenId = await contracts.playground.calculateId(identifier,sigHash,contracts.majorityWithoutToken.address,Alice.address)
            console.log('token id', tokenId)
            console.log('Alice address', Alice.address)
            console.log('majority voting contract', contracts.majorityWithoutToken.address)
            // await expect(contracts.playground.connect(Alice).start(votingParams, callback))
            //     .to.emit(contracts.badge, "Blaab")
            //     .withArgs(tokenId, Alice.address, sigHash, contracts.majorityWithoutToken.address, identifier)
            // await contracts.playground.connect(Alice).vote(0, APPROVE)
            // await contracts.playground.connect(Bob).vote(0, APPROVE)
            // let timestamp = (await ethers.provider.getBlock(await ethers.provider.getBlockNumber())).timestamp
            // await ethers.provider.send('evm_setNextBlockTimestamp', [timestamp + duration + 1]); 
            // await contracts.playground.connect(Charlie).implement(0, callback)
